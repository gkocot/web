...
async function playCameraStream(event) {
    var stream = await navigator.mediaDevices.getUserMedia({video_source: true, audio: false});
    video_source.src = '';
...
async function sendVideo() {
    let stream = video_source.captureStream();
    video_captured.srcObject = stream; 

    // let videoTrack = stream.getVideoTracks()[0];
    // rtpSender.replaceTrack(videoTrack);

    rtpSender = pc.addTrack(stream.getVideoTracks()[0], stream);
